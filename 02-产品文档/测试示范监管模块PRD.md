...
| V2.7 | 2026-06-30 | 产品经理 | 三类准入申请材料清单首位新增主体单位营业执照（材料1，系统自动从企业信息登记模块调取，无需手动上传），原材料顺序顺延：道路测试15→16、示范应用14→15、商业化试点16→17；同步更新新建申请弹窗、详情页材料展示及本清单项数、验收标准 | — |
...
| V2.9 | 2026-07-02 | 产品经理 | F15 异常状态预警全面重构，对齐参考文件《车辆状态预警页面需求》：(1) Tab 从 5 个调整为 7 个（新增车辆碰撞事件预警、临牌有效期预警独立 Tab；原车辆牌照识别系统故障 Tab 扩大为车载终端异常预警全链路监测）；(2) 新增 3.15.3 各预警类型触发条件章节，逐项描述 7 种预警的业务规则与多条件联合触发逻辑；(3) 新增 3.15.4 各 Tab 查询筛选项定义；(4) 功能清单 F15 描述同步更新 | — |
...
| -------- | ---- | ---- | --------- | ---- | --------------------------------------------------------------------------------------------------------- |
| 申请类型     | 文本显示 | —    | —         | 初次申请 | 固定显示                                                                                                      |
| 示范应用类型   | 单选   | 是    | 不可为空      | 示范应用 | 由A、B两段进行拼接，A段枚举值：主驾有人、自动驾驶。选择 " 自动驾驶示范应用 " 时展示额外前置条件提示和补充材料要求；B段枚举值：载人示范应用、载物示范应用、特种作业示范应用。示例：主驾有人-载人示范应用 |
...
| --------- | ---- | ---- | --------- | ---- | ------------------------------------------------------------------------------------------------------------- |
| 申请类型      | 文本显示 | —    | —         | 初次申请 | 固定显示                                                                                                          |
| 示范应用类型    | 单选   | 是    | 不可为空      |      | 由A、B两段进行拼接，A段枚举值：主驾有人、自动驾驶。选择 " 自动驾驶示范应用 " 时展示额外前置条件提示和补充材料要求；B段枚举值：载人商业化试点、载物商业化试点、特种作业商业化试点。示例：主驾有人-载人商业化试点 |
| 申请主体      | 下拉单选 | 是    | 数据来源：企业登记 | —    | —                                                                                                             |
