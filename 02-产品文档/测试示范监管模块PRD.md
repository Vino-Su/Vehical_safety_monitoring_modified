...
| 申请主体 | 文本输入框 | 支持模糊搜索                                              |
| 申请状态 | 多选下拉  | 草稿/待初审/待专班审核/待专家评审/待专题会审议/已退回/已撤回/待上传牌照/待牌照审核/牌照审核退回/已生效/已过期/已终止 |
| 申请时间 | 日期范围  | 可选起始日期和结束日期；分别按申请创建日期的下限和上限进行包含式筛选 |
...
**状态路径：** 草稿 → 待初审 → 待专班审核 → 待专家评审 → 待专题会审议 → 待上传牌照 → 待牌照审核 → 已生效 → 已过期；牌照审核不通过时进入牌照审核退回，修改后重新提交回到待牌照审核
...
| 4   | 申请主体  | company     | —                                             |
| 5   | 申请时间  | time        | —                                             |
| 6   | 当前节点  | currentNode | 第三方初审/市工作专班审核/专班审核确认/专家评审/专题会审议/待上传牌照/临时牌照审核/牌照审核退回/已完成/已退回 |
...
1. 用户点击列表“详情”→ 打开只读申请详情及流程信息；审批人点击列表“审批”→ 打开相同的申请详情及流程信息，并追加审批操作区。当前节点为“临时牌照审核”时，审批区展示本次牌照清单及牌照审核操作。
...
3. 点击 " 通过 "：
   ...
   - Toast 提示 " 审批通过 " + 刷新列表
   - 当前节点为“临时牌照审核”→ 申请状态变为“已通过”，节点结束并进入申请有效期
4. 点击 " 退回 "：
    - 当前节点字段必填（第三方初审、专家评审为备注，其他节点为审批意见）→ 状态变为 " 已退回 "
    - 提交前弹出二次确认；取消确认不改变申请状态
    - 第三方初审退回时至少存在一条材料批注；系统按材料编号、材料名称、问题说明生成问题清单，随退回结果通知申请主体并写入流程日志
    - 其他节点确认后通知申请主体，并将退回意见写入流程日志
    - 当前节点为“临时牌照审核”时，退回结论命名为“不通过”，审核意见必填；申请状态变为“牌照审核退回”，申请主体可修改牌照信息后重新提交审核
...
```
草稿 → 待初审 → 待专班审核 → 待专家评审 → 待专题会审议 → 待上传牌照 → 待牌照审核 → 已生效 → 已过期
                ↓ 退回          ↓ 退回        ↓ 退回          ↓ 退回
              已退回           已退回         已退回           已退回
```
...
车辆变更/新增车辆：草稿 → 待初审 → 待专班审核 → 待上传牌照 → 待牌照审核 → 已生效 → 已过期
```
...
- 用户须完成全部待登记车辆的信息后提交牌照审核。提交成功后申请状态变为“待牌照审核”，当前节点为第三方专业管理机构，申请不生效。
- 第三方专业管理机构对本次已登记牌照逐项核验后，对牌照信息整体作出“通过”或“不通过”结论。通过时申请状态变为“已生效”；不通过时必须填写牌照问题及修改要求，申请状态变为“牌照审核退回”，并向申请主体回传审核意见。
- 牌照审核退回时申请主体可从申请列表或申请详情进入“修改临时牌照”，仅允许编辑临时牌照号、有效期和牌照照片；修改并提交后重新进入“待牌照审核”，保留历次审核意见和处理记录。
...
| S07  | 待牌照审核  | 用户已完成全部待登记车辆牌照信息提交，等待第三方专业管理机构审核 | 否 |
| S08  | 牌照审核退回 | 第三方牌照审核不通过，等待申请主体按意见修改并重新提交 | 否 |
| S09  | 已生效    | 牌照审核通过或审批通过后无需登记牌照，申请进入有效期             | 否    |
| S10  | 已过期    | 当前时间超过申请有效期                   | 是    |
| S11  | 已退回    | 审批节点被退回，需修改后重新提交              | 是    |
| S12  | 已撤回    | 用户在审批前主动撤回申请                  | 是    |
| S13  | 已变更    | 原申请被变更申请替换覆盖，不再有效             | 是    |
| S14  | 已终止    | 触发系统自动终止或管理部门手动终止             | 是    |
...
| E16  | 提交牌照审核    | 测试主体 | 完成全部待登记车辆牌照号、有效期和照片后提交第三方审核 |
| E17  | 牌照审核通过/不通过 | 第三方专业管理机构 | 对本次牌照信息整体作出通过或不通过结论；不通过须填写修改意见 |
| E18  | 修改后重新提交牌照 | 测试主体 | 根据牌照审核意见修改信息并再次提交审核 |
...
stateDiagram-v2
    ...
    待上传牌照 --> 待牌照审核 : 提交牌照审核
    待牌照审核 --> 已生效 : 牌照审核通过
    待牌照审核 --> 牌照审核退回 : 牌照审核不通过
    牌照审核退回 --> 待牌照审核 : 修改后重新提交
    已生效 --> 已过期 : 到期
    已生效 --> 已变更 : 变更覆盖
    已生效 --> 已终止 : 资格终止
    已退回 --> 待初审 : 重新提交
...
| 待专家评审  | 专家评审退回  | 已退回    | 需填写备注            |
| 待专题会审议 | 专题会审议通过 | 待上传牌照  | 仅初次申请            |
| 待专题会审议 | 专题会审议退回 | 已退回    | 需填写审批意见          |
| 待上传牌照  | 提交牌照审核    | 待牌照审核 | 全部待登记车辆牌照号、有效期和照片录入并校验通过 |
| 待牌照审核 | 牌照审核通过 | 已生效 | 第三方专业管理机构核验通过 |
| 待牌照审核 | 牌照审核不通过 | 牌照审核退回 | 第三方必须填写牌照问题及修改要求 |
| 牌照审核退回 | 修改后重新提交牌照 | 待牌照审核 | 申请主体完成修改并通过全部校验 |
| 已生效    | 到期      | 已过期    | 系统定时任务触发         |
...
- [ ] 道路测试新增相同配置车辆申请仅上传“最新道路测试安全性自我声明”“新增相同配置车辆必要性说明”；车辆须与原申请型号、自动驾驶级别一致，并支持从备案人员档案选择新增安全员，原申请附件按实际初次申请材料清单只读关联
...
- [ ] 初次申请、涉及新增或替换车辆的变更申请、以及新增相同配置车辆申请审批通过后，可从列表或详情车辆信息进入同一临时牌照登记功能；弹窗按 VIN 码、型号、临时牌照号、有效期、牌照照片、操作展示当前申请车辆，支持按车辆编辑牌照号、有效期和照片；临牌号校验全平台唯一，全部待登记车辆校验通过后提交第三方牌照审核；第三方通过后申请才生效，不通过时必须填写修改意见并回流申请主体修改后重新提交；延期及不涉及车辆的变更不显示登记入口，已生效详情只读展示牌照信息
...
- [ ] 示范应用新增相同配置车辆申请仅上传“最新示范应用安全性自我声明”“新增相同配置车辆必要性说明”；车辆须与原申请型号、自动驾驶级别一致，并支持从备案人员档案选择新增安全员，原申请附件按实际初次申请材料清单只读关联
- [ ] 商业化试点新增相同配置车辆申请仅上传“最新商业化试点安全性自我声明”“新增相同配置车辆必要性说明”；车辆须与原申请型号、自动驾驶级别一致，并支持从备案人员档案选择新增安全员，原申请附件按实际初次申请材料清单只读关联；延期与变更材料分别按对应规则展示
- [ ] 车辆展示字段包含里程/违法/事故/保险
...
- [ ] 审批通过：正确流转到下一节点
- [ ] 审批退回：状态变为已退回；第三方初审、专家评审需填写备注，其他节点需填写审批意见
