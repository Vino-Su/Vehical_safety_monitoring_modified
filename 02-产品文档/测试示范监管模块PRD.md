...
> V3.6（2026-08-10）：F14 车辆运行监测的视频监控 Tab 移除车辆列表内的快速检索入口和无业务语义的标识，列表直接展示统一车辆数据的车牌号、所属企业、应用类型和运行状态，保留选车驱动视频工作台的联动能力；运行数据 Tab 的列表字段更新为临牌号、VIN码、所属企业、数据采集时间、经度、纬度、车辆驾驶模式、车辆速度、档位，详情补充车辆动态、车身信号与远程控制数据字段；自动驾驶状态 Tab 的列表字段更新为临牌号、VIN码、所属企业、数据采集时间、车辆驾驶模式、接管状态。F22 交通违法行为记录名称调整为”交通违法信息”。F13 轨迹回放与视频历史回放的时间轴精度从分钟升级到秒，当前时间显示、进度条滑块和时间范围筛选均支持秒级定位。F15 异常状态事件支持按事件分类、企业简称、临牌号、VIN码、处理状态和发生时间查询；统计、列表与导出按当前筛选结果联动，全部事件 Tab 均支持按当前页选择记录后导出；详情补充命中规则、实时快照及车辆数据查询、轨迹与视频回放联动。F11 在线反馈管理、F15 异常状态事件和 F22 交通违法信息的多条件查询采用高频条件默认可见、其余条件按需展开的交互方式；展开和收起不清除已录入条件，查询与重置均作用于全部条件。

> V3.7（2026-08-11）：F22 交通违法信息的导出入口与查询、重置统一归入查询操作区，导出范围仍遵循当前筛选条件；F11 在线反馈管理的导出入口与查询、重置统一归入查询操作区，导出范围仍遵循当前筛选条件或已选记录。
...
2. **Tab 页签区 + 数据列表区**：5 个 Tab（全部、待处理、处理中、已处理、已关闭），底部分页栏（见附录 A.1）
