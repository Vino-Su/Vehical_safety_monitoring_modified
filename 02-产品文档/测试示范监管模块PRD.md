...
| V4.9 | 2026-09-21 | 产品经理 | F04 道路测试类型枚举统一为“主驾有人”“自动驾驶”，申请、变更及详情展示均不附加驾驶位人员说明；道路测试初次申请查看详情的基本信息新增“道路测试类型”，延期、变更和新增相同配置车辆等派生申请详情的“原申请内容”同步展示关联初次申请的道路测试类型。F05 将“示范应用模式”更名为“示范应用类型”，枚举统一为“主驾有人”“自动驾驶”；原“示范应用类型”更名为“示范应用子类型”，枚举保持“载人示范应用”“载物示范应用”“特种作业示范应用”；新增申请、初次申请详情及延期、变更、新增相同配置车辆的原申请信息统一采用该字段口径；准入申请列表以“示范应用类型”列展示主驾有人/自动驾驶。F06 将原“商业化试点模式”更名为“商业化试点类型”，枚举为“主驾有人”“自动驾驶”；原“商业化试点类型”更名为“商业化试点子类型”，枚举为“载人商业化试点”“载物商业化试点”“特种作业商业化试点”；新增申请、初次申请详情及延期、变更、新增相同配置车辆的原申请内容均展示两级字段，列表分别展示商业化试点类型和商业化试点子类型。 | — |
| V5.0 | 2026-09-23 | 产品经理 | 可视化驾驶舱“车辆实时监控”模块标题后增加车辆列表入口，打开车辆列表弹窗；弹窗展示车辆总数、在线车辆、离线车辆汇总，支持按车牌号搜索，列表展示车牌号、所属企业、申请类型、驾驶模式、速度、在线状态和最后上报时间，并与驾驶舱深色弹窗视觉规范保持一致；F13 监控一张图明确图例条目清单及与图层开关的联动规则：图例仅展示当前已开启图层对应的条目，图层开关关闭时对应条目同步隐藏、重新开启后恢复显示，全部图例条目均不可见时图例整体隐藏，图例开关关闭后重新开启时按当前图层开关状态还原；图例条目覆盖车辆实时位置（自动驾驶模式、人工驾驶模式、测试中车辆、离线车辆）、异常事件标识（预警事件车辆、告警事件车辆）、事故点位、行驶轨迹、电子围栏（允许区域-开放、允许区域-暂停、允许区域-关闭、禁止通行区域）、开放道路（开放、暂停、关闭）、设施设备（充电桩、交通信号灯、路侧传感器）和历史事故；图层面板补充“历史事故”开关项（默认关）。 | — |
...
1. **基本信息区**：申请编号、申请类型、申请主体、道路测试类型、测试周期、道路测试项目；延期/变更/新增车辆时展示 " 本申请类型 " 和 " 关联原申请编号 "（可点击跳转），其只读“原申请内容”同步展示关联初次申请的道路测试类型
...
| 示范应用类型   | 单选   | 是    | 不可为空      | 主驾有人 | 枚举值：主驾有人、自动驾驶。选择“自动驾驶”时展示额外前置条件提示和补充材料要求 |
| 示范应用子类型 | 单选   | 是    | 不可为空      | 载人示范应用 | 枚举值：载人示范应用、载物示范应用、特种作业示范应用 |
...
仅示范应用类型为“自动驾驶”时展示，证明材料应覆盖申报规程第二条、第四条、第五条：
...
**延期申请：** 与道路测试延期表单结构统一（顶部提示条：延期申请·基于原申请编号；原申请信息只读：申请主体、原时间段、示范应用类型、示范应用子类型、示范应用项目；延期内容：原测试周期只读展示、延期时间段开始日期自动接续原截止日期+1 天且只读、结束日期可编辑、延期原因必填；材料：最新安全性自我声明、延期申请书、前期总结评估报告、延期必要性说明 + 原申请附件只读折叠）；示范应用类型、示范应用子类型、申请主体、项目负责人、联系方式、路段、车辆、人员只读；仅示范应用时间段可修改；日期校验同道路测试

- 原申请附件保持只读关联；本次需上传最新安全性自我声明、延期申请书、前期总结评估报告、延期必要性说明
- 提交流程：提交延期申请 → 第三方初审 → 市工作专班审核确认 → 已生效

**变更申请：** 申请主体和示范应用类型只读；示范应用子类型、示范应用车辆、示范应用驾驶员（安全员）、示范应用时间、示范应用路段或区域、转场路段、示范应用项目均可勾选变更，未勾选字段保留原值。原申请信息同时展示示范应用类型和示范应用子类型。
...
2. **列表字段**：申请编号、申请类型、申请主体、商业化试点类型、商业化试点子类型、原申请编号、运营车辆数、运营路段数、流程状态、当前环节、申请时间、有效期起止、操作
...
| 商业化试点类型  | 单选   | 是    | 不可为空      | —    | 枚举值：主驾有人、自动驾驶。选择“自动驾驶”时展示商业化试点自动驾驶前置条件 |
| 商业化试点子类型 | 单选   | 是    | 不可为空      | —    | 枚举值：载人商业化试点、载物商业化试点、特种作业商业化试点 |
| 申请主体      | 下拉单选 | 是    | 数据来源：企业登记 | —    | —                                                                                                                   |
...
**商业化试点类型前置条件展示规则：** 选择“主驾有人”或“自动驾驶”商业化试点类型时，仅展示申报规程第三十一条、第三十二条规定的主体、车辆、数量及场景前置条件；选择“自动驾驶”时追加自动驾驶类型专属佐证材料要求。前置条件的实际数据由历史准入和运行记录提供，申请人不手工录入时长、里程，也不进行确认勾选。
...
**初次申请与详情字段口径：** 初次申请表单、初次申请详情基本信息以及延期、变更、新增相同配置车辆详情中的原申请内容，均分别展示“商业化试点类型”和“商业化试点子类型”；前者枚举主驾有人、自动驾驶，后者枚举载人商业化试点、载物商业化试点、特种作业商业化试点，不再将两组枚举拼接为单一字段。

- 延期：与道路测试、示范应用延期表单结构统一（原申请信息只读：申请主体、原时间段、商业化试点类型、商业化试点子类型、商业化试点项目；延期内容：原测试周期只读展示、延期时间段开始日期自动接续原截止日期+1 天且只读、结束日期可编辑、延期原因必填）；商业化试点类型、商业化试点子类型、申请主体、路段、车辆、人员只读；仅商业化试点时间段可修改；须在原申请有效期结束前 3 个月内提出，延期时长一次不超过 6 个月；原申请附件保持只读关联，本次上传最新商业化试点安全性自我声明、商业化试点延期申请书、前期商业化试点评估报告、延期必要性说明等申请材料；提交流程：提交延期申请 → 第三方初审 → 市工作专班审核确认 → 已生效
- 变更：申请主体和商业化试点类型只读；商业化试点子类型、商业化试点车辆、商业化试点驾驶员（安全员）、商业化试点时间、商业化试点路段或区域、转场路段、商业化试点项目均可勾选变更，未勾选字段保留原值；商业化试点车辆、驾驶员（安全员）、路段或区域在勾选后按原申请清单预填表格，仅可通过对应既有选择弹窗新增，不提供自由文本录入；删除操作标记为拟删除且允许恢复，保留、新增、拟删除对象在同一张表中展示；提交时固化对象快照和行级状态，变更详情以业务字段表格只读展示；勾选后的任一清单有效对象被标记为空时阻止提交；变更原因为必填文本框；原声明只读关联并支持预览、下载，最新声明、变更理由及必要性说明、相应证明材料为本次申请的必传上传文件，支持替换、预览、下载；商业化试点时间段校验不得早于现有日期；提交流程：提交变更申请 → 第三方初审 → 市工作专班审核确认；不涉及车辆时直接生效，涉及新增或替换车辆时登记本次车辆临时牌照后生效
...
| 示范应用准入  | 申请编号、申请类别、申请主体、示范应用类型、示范应用子类型、示范应用时间段、申请时间 |
| 商业化试点准入 | 申请编号、申请类别、申请主体、商业化试点类型、商业化试点子类型、商业化试点时间段、申请时间 |
...
- 图例内容随图层开关联动：仅展示当前已开启图层对应的图例条目，图层开关关闭时对应条目同步隐藏，重新开启后恢复显示；全部图例条目均不可见时图例整体隐藏，任一图层重新开启后自动恢复显示；图例开关关闭时图例整体隐藏，开启后按当前图层开关状态还原
- 图例条目与图层开关的对应关系：

| 图层开关 | 对应图例条目 |
| ---- | ---- |
| 车辆实时位置 | 自动驾驶模式、人工驾驶模式、测试中车辆、离线车辆 |
| 异常事件标识 | 预警事件车辆、告警事件车辆 |
| 事故点位 | 事故点位 |
| 行驶轨迹 | 行驶轨迹 |
| 电子围栏 | 允许区域-开放、允许区域-暂停、允许区域-关闭、禁止通行区域 |
| 开放道路 | 开放道路、暂停道路、关闭道路 |
| 设施设备 | 充电桩、交通信号灯、路侧传感器 |
| 历史事故 | 历史事故 |
...
- 可视化驾驶舱“车辆实时监控”模块标题后提供 `>>` 入口，点击打开车辆列表弹窗；弹窗保持当前驾驶舱上下文，支持关闭、遮罩点击和 Esc 退出。
- 车辆列表弹窗顶部展示车辆总数、在线车辆、离线车辆汇总，提供车牌号模糊搜索；搜索结果为空时展示明确的无匹配提示。
- 列表字段为车牌号、所属企业、申请类型、驾驶模式、速度（km/h）、在线状态、最后上报时间；在线/离线状态同时使用文字与状态点表达。
...
| 示范应用类型 | 选择“自动驾驶” | 显示自动驾驶示范应用前置条件提示区域；显示自动驾驶示范应用补充材料组 |
| 商业化试点类型 | 选择 " 自动驾驶 " | 显示申报规程第三十一条、第三十二条的商业化试点前置条件说明，并追加自动驾驶类型专属佐证材料要求；不提供手工时长、里程录入或确认勾选 |
| 示范应用子类型 | 选择“载人示范应用” | 显示“座位险/人身意外险”材料项 |
| 商业化试点子类型 | 选择 " 载人商业化试点 " | 显示 " 座位险/人身意外险 " 材料项 |
| 商业化试点子类型 | 选择 " 载人商业化试点 " 或 " 载物商业化试点 " | 附件11第8项保险材料展示“搭载人员或货物的”额外险种要求；同时触发4.11节对应场景的动态校验 |
...
| 示范应用类型切换 | 主驾有人 / 自动驾驶 | 仅展示对应的 480 小时、1000 / 2000 公里、无交通违法及无有责事故前置条件说明；不提供手工时长、里程录入或确认勾选，不以该说明阻止提交 |
...
| 商业化试点类型切换 | 主驾有人 / 自动驾驶 | 仅展示第三十一条、第三十二条规定的主体、车辆、数量及场景前置条件；不提供手工时长、里程录入或确认勾选 |
...
- [ ] 示范应用类型正确展示主驾有人、自动驾驶；示范应用子类型正确展示载人示范应用、载物示范应用、特种作业示范应用
- [ ] 示范应用初次申请按附件1/6/7申请文件、附件8申请材料展示；安全性自我声明作为附件7上传
- [ ] 附件8完整展示规程原有13项顺序，营业执照自动关联，驾驶员（安全员）材料关联已选人员档案且不重复上传
- [ ] 示范应用类型为主驾有人时，仅展示拟申请路段和区域的道路测试达到480小时和1000公里、无交通违法、无由道路测试车辆方承担责任的交通事故的前置条件说明，不提供手工录入或勾选确认
- [ ] 示范应用类型为自动驾驶时，展示自动驾驶示范应用补充材料组及远程道路测试达到480小时和2000公里、无交通违法、无由道路测试车辆方承担责任的交通事故的前置条件说明，不提供手工录入或勾选确认；不适用道路测试首次10辆限制和40/100小时远程操控要求
- [ ] 示范应用初次申请详情的基本信息，以及延期、变更、新增相同配置车辆申请和详情中的原申请信息，均同时展示关联申请的示范应用类型与示范应用子类型
...
- [ ] 附件11完整展示规程原有12项顺序，营业执照自动关联，驾驶人（安全员）材料关联已选人员档案且不重复上传；安全保障方案独立于附件11，并包含专业论证和安全风险评估材料
 - [ ] 商业化试点类型切换仅展示第三十一条、第三十二条的前置条件；选择自动驾驶时展示对应专属佐证材料，不提供手工时长、里程录入或确认勾选
...
- [ ] 车辆展示字段包含里程/违法/事故/保险
 - [ ] 商业化试点初次申请详情基本信息，以及延期、变更、新增相同配置车辆详情中的原申请内容，均同时展示商业化试点类型与商业化试点子类型；三类申请与道路测试申请详情使用相同的流程上下文栏规则，标题栏不展示流程状态或当前环节，基本信息不重复展示流程字段
