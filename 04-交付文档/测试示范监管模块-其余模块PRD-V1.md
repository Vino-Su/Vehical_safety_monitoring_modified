...
4. 点击车辆图标，车辆图标会居中，并弹出**信息窗**：标题为“车牌号”- 所属企业名称 - 在线状态，信息窗分为两个 tab 标签。
	...
	- 异常状态 tab 页：
		...
		- 异常卡片支持查看回放，点击回放按钮，会关闭当前弹窗，同时打开轨迹弹窗和视频弹窗，时间范围条件为【异常事件开始时间前 1min，异常事件结束时间/当前时间】（事件未结束，取当前时间，事件已结束，取结束时间），车辆为异常事件关联的车辆。地图左上角显示异常事件信息卡片信息。点击事件信息卡片“退出回看”，则退出当前模式，回到正常地图页面，关闭轨迹播放进度条、右侧数据面板切换至异常事件 tab
...
3. 地图下方为轨迹控制面板，包含时间轴、查询和展示区，需具备以下功能
	...
	- 视频回看按钮：点击后同步打开该车辆的视频画面，即右侧数据面板切换为视频 tab 页并选中该车辆，同步时间轴的时间范围和当前时间

**实时视频弹窗**

1. 触发条件：点击车辆信息页的“实时视频”按钮或者车辆状态列表中的视频按钮或者点击右侧数据面板视频 tab 标签
2. 视频 tab 窗口：
...
1. 数据源为车辆的异常事件，包括预警和告警的事件，为右侧数据面板的默认选中 tab
...
3. 卡片信息：
	...
	- 触发规则：选中卡片后显示，默认不显示
	- 处理状态：选中卡片后显示，默认不显示
4. 选中卡片后，地图会触发选中车辆图标行为，车辆信息弹窗切换至异常事件 tab
...
页面左侧为车辆列表，显示车辆临牌号、所在公司、测试示范类型；默认选中排序在前的车辆

页面右侧为该车辆对应的所有摄像头信息

- 支持切换摄像头展示效果，如单路、4 路或者 9 路
- 支持历史回放和实时播放两种模式
- 支持全屏显示
- 单个摄像头支持截图和录像
...
展示智能网联汽车的当前运行状态数据，同一辆车仅保留最新一条数据即可，该数据需要定期刷新

列表字段：临牌号、VIN 码、所属企业、数据采集时间、经度、纬度、车辆驾驶模式、车辆速度、档位

详情页字段：临牌号、VIN 码、所属企业、数据采集时间、经度、纬度、车辆驾驶模式、接管状态、DMS 状态、执行最小风险策略原因、车辆速度、车辆纵向加速度、车辆横向加速度、车辆横摆角速度、车辆侧倾角速度、车辆航向角、失效类型、自适应前照明系统状态、近灯光状态、远灯光状态、危险警告信号状态、制动灯状态、左转向信号灯状态、右转向灯状态、车辆喇叭信息、加速踏板开度比例、制动踏板开度比例、制动踏板状态、转向盘角度、档位、雨刮、远程控制指令长度、远程控制指令内容

#### 5.2.4 自动驾驶状态

展示接入平台中的车辆最新的自动驾驶的数据，同一辆车仅保留最新一条数据即可，该数据需要定期刷新

列表字段：临牌号、VIN 码、所属企业、数据采集时间、车辆驾驶模式、接管状态

#### 5.2.5 传感器数据上报

展示接入平台中的车辆最新的自动驾驶的数据，同一辆车仅保留最新一条数据即可，该数据需要定期刷新

列表字段：临牌号、VIN 码、所属企业、数据采集时间、车辆纵向加速度、车辆横向加速度、车辆横摆角速度、车辆侧倾角速度、车辆航向角

#### 5.2.6 里程统计

以日为单位，对车辆的里程数进行统计，同一辆车仅保留最新一条数据即可，该数据需要定期刷新

列表字段：临牌号、VIN 码、所属企业、数据更新时间、自动驾驶里程 (km)、人工驾驶里程 (km)、总里程 (km)

#### 5.2.7 电池状态监测

展示接入平台中的车辆的电池状态数据（仅针对新能源车辆），同一辆车仅保留最新一条数据即可，该数据需要定期刷新

列表字段：临牌号、VIN 码、所属企业、数据采集时间、总电压 (V)、总电流 (A)、温度 (℃)、SOC(%)、SOH(%)

#### 5.2.8 车辆故障信息

展示接入平台中的车辆的故障告警数据，同一辆车仅保留最新一条数据即可，该数据需要定期刷新

列表字段：临牌号、VIN 码、所属企业、数据采集时间、设备编号、故障码、故障描述、故障等级、所属系统
...
预警条件参照异常事件规则配置中的车辆运行异常预警事件的规则

列表字段：复选框、事件编号、事件分类、临牌号、VIN 码、所属企业、事件描述（对应异常预警事件中的规则子类型）、发生时间、处理状态、操作

详情字段：

- 基础信息：事件编号、事件分类、临牌号、VIN 码、所属企业、事件描述（对应异常预警事件中的规则子类型）、发生时间、处理状态
- 触发条件：具体规则
- 实时快照：车辆速度、车辆驾驶模式、车辆纵向加速度、车辆横向加速度、挡位
	- 更多数据按钮：点击后会将车牌、VIN 码、时间范围（异常事件发生时间 -20s，异常事件发生事件 +10s）等搜索条件传入监测数据分析 - 车辆数据查询 - 车辆实时数据查询页面，筛选出该车辆异常前后的所有数据
	- 回放按钮：点击后，将车牌、VIN 码、时间范围（异常事件发生时间 -20s，异常事件发生事件 +10s）等搜索条件传入打开监控一张图页面，进行轨迹播放和视频回看
- 处理信息
#### 5.3.3 车辆碰撞事件预警

预警条件参照异常事件规则配置中的车辆碰撞事件预警的规则

列表字段：事件编号、事件分类、临牌号、VIN 码、所属企业、事件描述（对应异常预警事件中的规则子类型）、发生时间、处理状态、操作
...
- 基础信息：事件编号、事件分类、临牌号、VIN 码、所属企业、事件描述（对应异常预警事件中的规则子类型）、发生时间、处理状态
- 触发条件：具体规则、临牌有效期起止
- 实时快照：数据采集时间、车辆速度、车辆驾驶模式、车辆纵向加速度、车辆横向加速度、挡位
	- 更多数据按钮：点击后会将车牌、VIN 码、时间范围（异常事件发生时间 -20s，异常事件发生事件 +10s）等搜索条件传入监测数据分析 - 车辆数据查询 - 车辆实时数据查询页面，筛选出该车辆异常前后的所有数据
	- 回放按钮：点击后，将车牌、VIN 码、时间范围（异常事件发生时间 -20s，异常事件发生事件 +10s）等搜索条件传入打开监控一张图页面，进行轨迹播放和视频回看
- 处理信息
#### 5.3.6 电子围栏越界告警

预警条件参照异常事件规则配置中的电子围栏越界告警的规则
