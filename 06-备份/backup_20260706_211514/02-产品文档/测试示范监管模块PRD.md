...
1. **页面标题**：" 异常状态预警 "
2. **统计卡片区**（4 列）：未处理预警数、正在处理预警数、已处理预警数、累计预警总数
3. **Tab 页签区**（7 个）：车辆运行异常预警、车辆碰撞事件预警、车载终端异常预警、临牌有效期预警、电子围栏越界预警、测试时段合规性预警、安全员到岗状态预警
4. **查询筛选区**：预警类型、车辆号牌/VIN、处理状态、发生日期
5. **数据列表区**：预警编号、车辆号牌/VIN、预警内容描述、预警详情、发生时间、处理状态
