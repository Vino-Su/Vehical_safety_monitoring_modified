...
| V2.6 | 2026-06-27 | 产品经理 | 补充 4.15 状态机设计章节：规范化描述准入申请主状态机（含初次申请、延期/变更/新增车辆派生申请）、派生申请与原申请联动状态机、在线反馈状态机、资格终止状态机、信息发布状态机、电子围栏状态机，含状态集合、事件集合、Mermaid 状态图、状态转换矩阵及通用约束 | — |

| V2.7 | 2026-06-30 | 产品经理 | 三类准入申请材料清单首位新增主体单位营业执照（材料 1，系统自动从企业信息登记模块调取，无需手动上传），原材料顺序顺延：道路测试 15→16、示范应用 14→15、商业化试点 16→17；同步更新新建申请弹窗、详情页材料展示及本清单项数、验收标准 | — |

| V2.8 | 2026-07-01 | 产品经理 | F11 在线反馈管理拆分为公众侧与管理侧两个独立子功能：(1) 公众侧（3.11.1）面向申请主体和社会公众，仅保留 " 提交问题/查询/跟踪 " 能力，移除 " 受理机构 " 列表列与所有管理部门操作按钮；(2) 管理侧（3.11.2）在测试服务管理模块下新增 " 在线反馈管理 " 页面，仅管理部门（第三方专业管理机构、政府监管部门）可见，含 5 个 Tab（全部/待处理/处理中/已处理/已关闭）、列表、详情、受理回复/继续回复/关闭、操作日志；(3) 新增列表字段 " 最新回复时间 "，由回复列表前端计算，无需新建后端字段；(4) 状态机（4.15.4）、编号规则（4.5）、数据结构保持不变；(5) 功能清单、验收标准、附录同步更新；本期不做统计卡片/批量操作/转派/认领/内部备注/消息推送 | — |

| V2.9 | 2026-07-02 | 产品经理 | 同日合并更新：(1) F15 异常状态预警全面重构，对齐参考文件《车辆状态预警页面需求》，Tab 从 5 个调整为 7 个并补充触发条件与筛选项；(2) F13 监控一张图同步异常状态事件新口径，取消严重/一般分级展示，改为按预警/告警两类异常事件进行统计、筛选、地图标识、右侧队列和车辆详情展示；(3) 功能清单与 F13 验收标准同步更新 | — |

| V3.0 | 2026-07-06 | 产品经理 | F22 交通违法行为记录高保真页面落地：(1) 新增 violation.html 页面，菜单位于「异常状态事件」与「事故数据上报」之间；(2) 含 7 项查询筛选、10 列数据列表（含复选框）、分页、详情弹窗（800px，4 区块 22 字段）；(3) 违法取证类型三态标签（电子抓拍/现场交警查处/视频举报）；(4) 导出按钮仅管理员可见；(5) common.js 菜单注册同步更新 | — |

| V3.1 | 2026-07-06 | 产品经理 | 同日合并更新：F19 电子围栏管理围栏类型与限速配置改造：(1) 新增「围栏类型」字段（允许通行区域 / 禁止通行区域），允许通行区域联动显示限速配置（是否限速 + 限速值 1-200 km/h）和"支持申请主体申请该区域"勾选，禁止通行区域隐藏以上两项；(2) 移除「允许误差范围」字段；(3) 地图样式改为双维度（类型 + 状态），禁止通行区域统一红色虚线，允许通行区域关闭状态改为灰色（非红色）避免与禁区混淆；(4) 列表卡片增加围栏类型行（含限速信息、待配置矢量提示）；(5) 右侧信息面板增加围栏类型行和限速配置行；(6) 更新图例，禁止通行区域单独一栏；(7) 禁止通行区域新增「是否限期」及有效期范围（开始~结束日期），适用于重大活动期间临时禁行场景，到期后自动失效；(8) 所有围栏类型新增「管控要求说明」多行文本字段（非必填，最多 500 字）；(9) 同步更新 fence.html 高保真页面、字段联动规则、验收清单 | — |
| V3.2 | 2026-07-06 | 产品经理 | F08 审批管理页面（04-交付文档 §5.9 F09）高保真落地：(1) 列表列对齐 §5.9.1 顺序（申请编号、业务类型、申请类型、申请主体、申请时间、当前节点、审批状态、操作），"申请类别"更名为"业务类型"，"申请子类型 + 示范类型"两列合并为"申请类型"一列（详情页保留示范类型展示），业务类型展示值取枚举值（道路测试 / 示范应用 / 商业化试点），不带"申请"二字；(2) 查询筛选区按 §5.9.1 新增"申请类型"下拉、"申请时间"日期范围控件（起止日期），与现有"申请编号/业务类型/申请主体"共同构成 5 项筛选；(3) 操作列"审批"按钮按 §5.9.2 实现：审批中状态显示"审批"按钮，已通过/已退回仅显示"详情"；(4) 审批详情弹窗与审批弹窗均支持全屏切换；(5) 审批弹窗底部固定审批操作栏（流程节点信息：当前节点/下一节点、审批意见、附件、审批动作：退回/通过/取消）；(6) 同步更新 approve.html、字段联动规则与验收清单 | — |

| V3.3 | 2026-08-04 | 产品经理 | 同日合并更新：F14 查询筛选、运行数据 Tab 字段调整；F15 异常状态事件查询条件全局化；F18 信息发布管理查询与列表调整；F13/F14 视频、轨迹与数据联动改造：(1) 两页共用车辆、摄像头和时间数据，运行监测车辆列表覆盖全部可监控车辆；(2) 实时视频、历史录像和轨迹回放按同一时间指针联动，视频与轨迹均可反向定位；(3) 支持摄像头主画面切换、多路查看、历史录像查询、进度定位、倍速和受权限控制的导出；(4) 监控一张图保留异常事件态，并可切换至车辆视频态。同步更新 F13/F14 验收标准 | — |
| V3.4 | 2026-08-05 | 产品经理 | 同日合并更新：(1) F13 视频工作台优化：地图视频态支持从统一车辆数据中选择车牌号并同步视频源和地图选中车辆；历史录像保留查询、定位与播放能力，不提供录像导出；(2) F18 信息发布管理列表字段统一：全部/草稿箱/待审核/已发布/已下架五个 Tab 列表字段与"全部"Tab 完全一致，取消各状态 Tab 差异化时间列（提交审核时间/发布时间/下架时间），操作按钮仍按发布状态动态显示；(3) F13 视频操作范围细化：截图和本地录像均针对指定摄像头画面，全屏状态可直接退出；历史回放不展示录像片段数量；(4) F18 新增发布机构字段：新建/编辑表单增加发布机构下拉框（必填，枚举：市工作专班办公室/市交通运输局/市交管部门/安全管理办公室/平台技术支持组），创建人改为系统自动获取当前登录用户且只读不可编辑；F18 五个 Tab 列表与详情弹窗同步增加发布机构列（列表共 9 列）；F21 信息公开三个 Tab 列表均增加发布机构列，办事指南原"发布部门"统一更名为"发布机构"，三类详情弹窗同步展示发布机构 | — |
...
| 8   | 申请状态   | status       | 110px | 标签展示（见状态颜色规范）              |
| 9   | 测试周期   | validPeriod  | 180px | 格式：YYYY-MM-DD 至 YYYY-MM-DD |
| 10  | 申请时间   | createTime   | 110px | 格式：YYYY-MM-DD HH-MM-SS     |
| 11  | 操作     | —            | 280px | 见操作列按钮                     |
|     |        |              |       |                            |
...
| 对应申请类型                  | 状态     | 定义                               | 标签颜色 | CSS 类              |
| ----------------------- | ------ | -------------------------------- | ---- | ------------------ |
| 初次申请                    | 草稿     | 用户保存申请为草稿，未提交                    | 灰色   | ant-tag-default    |
| 初次申请、延期申请、续期申请、新增相同配置申请 | 待初审    | 用户提交申请流程，下一节点（专班）尚未处理            | 蓝色   | ant-tag-processing |
| 初次申请、延期申请、续期申请、新增相同配置申请 | 待专班审核  | 第三方专业机构审核已通过，下一节点（专班）未处理         | 蓝色   | ant-tag-processing |
| 初次申请                    | 待专家评审  | 专班审核已通过，专家评审尚未通过                 | 蓝色   | ant-tag-processing |
| 初次申请                    | 待专题会审议 | 专家评审已通过，专题会议审批尚未通过               | 蓝色   | ant-tag-processing |
| 初次申请、延期申请、续期申请、新增相同配置申请 | 已退回    |                                  | 红色   | ant-tag-error      |
| 初次申请、延期申请、续期申请、新增相同配置申请 | 已撤回    | 用户提交申请后，在下一节点未处理前，主动撤回申请         | 灰色   | ant-tag-default    |
| 初次申请、延期申请、续期申请、新增相同配置申请 | 待上传牌照  | 专题会议审批通过，用户尚未上传牌照信息              | 橙色   | ant-tag-warning    |
| 初次申请、延期申请、续期申请、新增相同配置申请 | 已生效    | 用户已上传牌照信息，申请流程结束，且该流程当前处于申请的时间段内 | 绿色   | ant-tag-success    |
| 初次申请、延期申请、续期申请、新增相同配置申请 | 已过期    | 对于已生效的流程，当前时间已经超过申请的时间段          | 灰色   | ant-tag-default    |
| 初次申请、延期申请、续期申请、新增相同配置申请 | 已变更    | 对于已生效的流程，用户进行了变更申请，且变更申请已通过      | 蓝色   | ant-tag-blue       |
| 初次申请、延期申请、续期申请、新增相同配置申请 | 已终止    | 由于触发系统自动终止或者管理部门手动终止了相应流程        | 红色   | ant-tag-error      |
...
| 测试车辆    | 表格多选 | 是    | 至少选择 1 辆    | —    | 弹窗选择，数据来源：车辆登记，列：VIN 码、生产企业、车辆型号、自动驾驶级别；选择后支持填写临时牌照号及有效期 |
| 测试人员    | 表格多选 | 是    | 至少选择 1 人    | —    | 弹窗选择，数据来源：人员登记，列：姓名、性别、年龄、证件类型、证件号码                      |
...
| 序号   | 材料名称                                                                                                                                                                       | 是否必填 |
| ---- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- |
| 材料 1  | 道路测试主体单位营业执照（系统自动从企业信息登记模块调取，无需手动上传） | 是    |
| 材料 2  | 智能网联汽车道路测试安全性自我声明                                                                                                                                                          | 是    |
| 材料 3  | 交通事故责任承诺书                                                                                                                                                                  | 是    |
| 材料 4  | 道路测试主体单位法人授权委托书                                                                                                                                                            | 是    |
| 材料 5  | 道路测试车辆的自动驾驶功能等级声明以及自动驾驶功能对应的设计运行条件说明                                                                                                                                       | 是    |
| 材料 6  | 道路测试车辆设计运行范围与拟进行道路测试路段、区域内各类交通要素对应关系说明                                                                                                                                     | 是    |
| 材料 7  | 属国产机动车的，提供机动车整车出厂合格证，对未进入公告车型的可提供出厂合格证明和国家认可的第三方检测机构出具的相应车型强制性检验报告；属进口机动车的，提供进口机动车辆强制性产品认证证书、随车检验单和货物进口证明书，对未取得进口机动车辆强制性产品认证证书的可提供车辆满足安全运行条件的声明和国家认可的第三方检测机构出具的相应车型强制性检验报告 | 是    |
| 材料 8  | 自动驾驶功能说明及其未降低车辆安全性能的证明                                                                                                                                                     | 是    |
| 材料 9  | 襄阳市机动车安全技术检验机构出具的机动车安全技术检验合格证明                                                                                                                                             | 是    |
| 材料 10  | 具有网联功能或远程控制功能的监控平台网络安全风险评估结果及风险应对措施证明                                                                                                                                      | 否    |
| 材料 11 | 道路测试主体自行开展的模拟仿真测试与测试区（场）等特定区域实车测试的说明材料                                                                                                                                     | 是    |
| 材料 12 | 国家或省市认可的第三方检测机构出具的智能网联汽车自动驾驶功能委托检验报告                                                                                                                                       | 是    |
| 材料 13 | 经第三方专业管理机构评审通过的道路测试方案                                                                                                                                                        | 是    |
| 材料 14 | 交通事故责任强制险凭证及每车不低于 500 万元的交通事故责任保险凭证或事故赔偿保函                                                                                                                                   | 是    |
| 材料 15 | 道路测试主体承诺安装监管装置、接受第三方专业管理机构日常监管的承诺书                                                                                                                                           | 是    |
| 材料 16 | 拟接入智能网联车辆运行管理服务平台的数据清单及数据接入承诺书和服从管理承诺书                                                                                                                                     | 是    |

> 注：材料 1（道路测试主体单位营业执照）作为清单首项展示，由系统自动从企业信息登记模块调取，无需手动上传；驾驶员（安全员）信息在用户选取特定人员后自动带入其所需信息（身份证、驾驶证、劳动合同等），无需在申请材料中重复提交。
...
1. **页面标题区**：标题 " 示范应用准入申请 "
2. **列表字段**：申请编号、申请类型、示范应用类型、申请主体、示范应用类型、原申请编号、应用车辆数、应用路段数、申请状态、申请时间、有效期起止、操作
...
> 注：材料 1（示范应用主体单位营业执照）作为清单首项展示，由系统自动从企业信息登记模块调取，无需手动上传；驾驶员（安全员）信息在用户选取特定人员后自动带入其所需信息（身份证、驾驶证、劳动合同等），无需在申请材料中重复提交。
...
| --------- | ---- | ---- | --------- | ---- | ------------------------------------------------------------------------------------------------------------------- |
| 申请类型      | 文本显示 | —    | —         | 初次申请 | 固定显示                                                                                                                |
| 示范应用类型    | 单选   | 是    | 不可为空      |      | 由 A、B 两段进行拼接，A 段枚举值：主驾有人、自动驾驶。选择 " 自动驾驶示范应用 " 时展示额外前置条件提示和补充材料要求；B 段枚举值：载人商业化试点、载物商业化试点、特种作业商业化试点。示例：主驾有人 - 载人商业化试点 |
| 申请主体      | 下拉单选 | 是    | 数据来源：企业登记 | —    | —                                                                                                                   |
...
> 注：材料 1（商业化试点主体单位营业执照）作为清单首项展示，由系统自动从企业信息登记模块调取，无需手动上传；驾驶人（安全员）信息在用户选取特定人员后自动带入其所需信息（身份证、驾驶证、劳动合同等），无需在申请材料中重复提交。同一车辆在示范应用申请时已提交材料且在有效期内的可不再重复提交。
...
- 审批状态 = 审批中：操作列同时显示"详情"和"审批"按钮
- 审批状态 = 已通过 / 已退回：操作列仅显示"详情"按钮，不显示"审批"按钮

> "我的待办"Tab = 审批状态 = 审批中（不区分处理人角色）。审批详情弹窗与审批弹窗均支持全屏切换。
...
**可见角色：** enterprise（企业用户）、public（公众访客）；admin 在该菜单项下仍可见入口，但页面内不展示任何管理部门操作按钮，所有回复/关闭操作统一在 3.11.2 管理侧页面完成
...
| 元素   | 类型    | 配置说明                              |
| ---- | ----- | --------------------------------- |
| 处理状态 | 下拉单选  | 枚举值：全部、待处理、处理中、已处理、已关闭          |
...
4. 详情弹窗底部仅显示 " 关闭 " 按钮，不提供任何管理部门操作按钮（受理/继续回复请管理部门在 3.11.2 管理侧操作）
...
1. **页面标题区**：左侧标题 " 在线反馈管理 "，右侧 " 导出 " 按钮
2. **查询筛选区**：查询条件与按钮同行排列
3. **Tab 页签区 + 数据列表区**：5 个 Tab（全部、待处理、处理中、已处理、已关闭），底部分页栏（见附录 A.1）
...
| ---- | -------------------------- | ----------------- |
| 查看详情 | 打开反馈详情弹窗                  | 全部               |
| 受理回复 | 打开受理回复弹窗，预填受理人=当前用户、受理机构=当前机构 | 状态=待处理           |
| 继续回复 | 打开受理回复弹窗                  | 状态=处理中           |
| 关闭   | 二次确认后关闭反馈                 | 状态=待处理 或 状态=处理中 |

> 已处理、已关闭 状态的反馈仅保留 " 查看详情 " 按钮，其他按钮置灰或不显示。
...
- 已处理的反馈不可再次回复
- 所有回复记录在反馈详情中按时间线展示
- 关闭操作（独立于回复）将反馈状态变为 " 已关闭 "，用户不能再追加问题，但管理部门仍可查看历史
...
4. 用户点击 " 受理回复 "（待处理状态）→ 打开受理回复弹窗（处理状态默认 " 处理中 "） → 填写回复内容 → 点击 " 提交回复 "：
   - 校验成功 → 保存回复记录 + 状态更新为 " 处理中 " + Toast 提示 " 已受理该反馈 " + 关闭弹窗 + 刷新列表
   - 校验失败 → 错误提示 + 弹窗不关闭
5. 用户点击 " 继续回复 "（处理中状态）→ 同上；可在弹窗中将处理状态切换为 " 已处理 "，提交后反馈状态变为 " 已处理 "
6. 用户点击 " 关闭 " → 二次确认弹窗（" 关闭后用户不能再追加问题，确认关闭？"）→ 确认后状态变为 " 已关闭 " + Toast 提示 + 刷新列表
7. 用户点击 " 导出 " → 按当前筛选条件导出 Excel（见附录 A.3）
...
| 遮罩   | 半透明黑色，点击遮罩不关闭   |
| 标题   | 关闭确认           |
| 内容   | " 关闭后用户不能再追加问题，确认关闭？" |
| 按钮   | 取消 + 确认关闭       |

##### 3.11.6.8 权限规则

| 角色            | 查看列表 | 查看详情 | 受理回复 | 继续回复 | 关闭 | 导出 |
| ------------- | ---- | ---- | ---- | ---- | -- | -- |
| admin（监管人员）  | 是    | 是    | 是    | 是    | 是  | 是  |
| enterprise    | 否    | 否    | 否    | 否    | 否  | 否  |
| public        | 否    | 否    | 否    | 否    | 否  | 否  |
...
- 点击车辆图标弹出信息窗：标题为车辆详情 -“车牌号”，显示字段为：VIN、所属企业、驾驶模式、实时速度、加速度、航向角、档位、自动驾驶等级、在线状态、最后上报时间、当前异常事件（如果无异常事件，可不显示该字段）
- 信息窗底部操作按钮：查看轨迹、实时视频
- 支持车辆搜索定位聚焦
...
- 实时视频：默认展示选中车辆的当前摄像头画面；地图视频态支持从统一车辆数据中选择车牌号，切换后同步视频源与地图选中车辆；可从该车辆的摄像头集合选择并切换主画面；截图和本地录像均针对指定摄像头画面；支持全屏查看并可直接退出全屏；车辆离线或摄像头不可用时提示“视频流不可用”
- 历史回放：用户可在同一视频工作台进入历史回放，选择日期和时间范围后查询录像；支持进度定位、播放/暂停、倍速和回到实时，不展示录像片段数量
...
- 车辆列表基于平台统一车辆数据展示，支持按车牌号、所属企业或 VIN 模糊检索；选择车辆后，所有视频视图切换至该车辆
- 视频工作台默认提供多路查看，支持切换至单路主画面并从摄像头集合中选择画面；如无视频流提示“视频流不可用”
- 实时视频和历史回放共用同一工作台；截图和本地录像均以单个摄像头画面为对象；历史回放支持日期与时间范围查询、片段定位、倍速和回到实时，不展示录像片段数量且不提供录像导出
...
展示车辆自动驾驶功能运行状态。列表字段：车牌号、VIN码、所属企业、自动驾驶等级、当前速度、当前模式、最后数据上报时间、操作
...
1. **页面标题**：" 异常状态事件 "
2. **全局查询筛选区**（位于统计卡片区上方，对所有 Tab 统一生效）：事件分类（全部/预警/告警）、所属企业（模糊搜索）、车牌号、VIN码、处理状态、发生日期
3. **统计卡片区**（4 列）：未处理事件数(今日)、正在处理事件数、已处理事件数(今日)、累计事件总数
4. **Tab 页签区**（7 个）：车辆运行异常预警、车辆碰撞事件预警、车载终端异常预警、临牌有效期告警、电子围栏越界告警、测试时段合规性告警、安全员到岗状态告警
5. **数据列表区**：事件编号、事件分类、车牌号、VIN码、所属企业、事件内容描述、发生时间、处理状态、操作
...
| Tab                       | 筛选字段                                                           |
| ------------------------- | -------------------------------------------------------------- |
| 车辆运行异常预警                | 预警类型（全部/车速超限/加速度异常/偏离路线）、车牌号、VIN 码、处理状态、发生日期            |
| 车辆碰撞事件预警                | 碰撞严重程度（全部/轻微/一般/严重）、车牌号、VIN 码、处理状态、发生日期                 |
| 车载终端异常预警                | 故障类型（全部/数据中断/传感器故障/系统失效/导航异常）、车牌号、VIN 码、处理状态、发生日期   |
| 临牌有效期预警                 | 车牌号、VIN 码、处理状态、发生日期                                        |
| 电子围栏越界预警                | 越界方向（全部/驶出围栏/驶入禁行区）、车牌号、VIN 码、处理状态                     |
| 测试时段合规性预警               | 车牌号、VIN 码、处理状态、发生日期                                        |
| 安全员到岗状态预警               | 车牌号、VIN 码、处理状态、发生日期                                        |
...
2. **查询筛选区**：标题、信息类型、发布状态（枚举：全部/草稿/待审核/已发布/已下架）
3. **Tab 页签区**：全部、草稿箱、待审核、已发布、已下架
4. **数据列表区**：通知公告编号、标题、信息类型、发布机构、创建人、目标受众、发布状态、最后修改时间/创建时间、操作按钮（全部 Tab 页签——全部/草稿箱/待审核/已发布/已下架——列表字段完全一致；操作按钮按发布状态动态显示：草稿箱显示编辑/删除，待审核显示查看/撤回，已发布显示查看/编辑/撤回，已下架仅显示查看）
...
| ---------- | ----- | ---- | ---- | ---------------------------- | -------------------------------- |
| 电子围栏名称     | 文本输入  | 是    | 不可为空 | —                            | —                                |
| 围栏类型       | 下拉单选  | 是    | 不可为空 | 允许通行区域                      | 枚举值：允许通行区域、禁止通行区域；联动控制下方限速配置、申请字段及限期字段的显隐  |
| 电子围栏状态     | 下拉单选  | 是    | 不可为空 | 待配置矢量                      | 枚举值：开放、暂停、关闭、待配置矢量               |
| 是否限速       | 勾选框   | 否    | —    | 未勾选                          | 仅围栏类型 = 允许通行区域时显示；勾选后限速值输入框可用 |
| 限速值 (km/h) | 数字输入  | 否    | 1~200 正整数 | —                     | 仅是否限速勾选时可输入，否则置灰；超出 1~200 范围红边提示  |
| 车辆越界报警     | 勾选框   | 否    | —    | 勾选                           | —                              |
| 已停用车辆自动移除  | 勾选框   | 否    | —    | 勾选                           | —                              |
| 支持申请主体申请该区域 | 勾选框 | 否    | —    | 勾选                           | 仅围栏类型 = 允许通行区域时显示；勾选后该围栏可被准入申请时选取 |
...
| ---------- | ----- | ---- | ---- | ---------------------------- | -------------------------------- |
| 电子围栏名称     | 文本输入  | 是    | 不可为空 | —                            | —                                |
| 围栏类型       | 下拉单选  | 是    | 不可为空 | —                            | 枚举值：允许通行区域、禁止通行区域；联动控制下方限速配置、申请字段及限期字段的显隐  |
| 电子围栏状态     | 下拉单选  | 是    | 不可为空 | —                            | 枚举值：开放、暂停、关闭、待配置矢量               |
| 是否限速       | 勾选框   | 否    | —    | 回显已有数据                       | 仅围栏类型 = 允许通行区域时显示；勾选后限速值输入框可用 |
| 限速值 (km/h) | 数字输入  | 否    | 1~200 正整数 | 回显已有数据               | 仅是否限速勾选时可输入，否则置灰；超出 1~200 范围红边提示  |
| 车辆越界报警     | 勾选框   | 否    | —    | 回显已有数据                       | —                              |
| 已停用车辆自动移除  | 勾选框   | 否    | —    | 回显已有数据                       | —                              |
| 支持申请主体申请该区域 | 勾选框 | 否    | —    | 回显已有数据                       | 仅围栏类型 = 允许通行区域时显示；勾选后该围栏可被准入申请时选取 |
...
| 触发字段   | 触发条件                | 联动行为                                                              |
| ------ | ------------------- | ----------------------------------------------------------------- |
| 围栏类型   | 选择 " 禁止通行区域 "      | 隐藏限速配置（是否限速 + 限速值）和支持申请主体申请该区域勾选项；显示是否限期及有效期范围字段；规则配置仅保留车辆越界报警、已停用车辆自动移除  |
| 围栏类型   | 选择 " 允许通行区域 "      | 显示限速配置和支持申请主体申请该区域勾选项；隐藏是否限期及有效期范围字段               |
...
| 规则配置   | 勾选 " 已停用车辆自动移除 "  | 车辆停用后自动从该围栏监控对象中移除                                             |

#### 3.19.5 交互流程

1. 用户点击 "+ 新建电子围栏 " → 打开新建围栏弹窗（填写基本信息）
2. 填写围栏名称、围栏类型（允许通行区域 / 禁止通行区域）、电子围栏状态、限速配置（仅允许通行区域时显示）、规则配置 → 点击 " 保存基本信息并进入图形绘制阶段 "：
...
| 限速配置   | 仅允许通行区域时显示；不限速 / 限速 N km/h（如 " 限速 60 km/h"）                  |
| 规则配置   | 列出勾选项：车辆越界报警、已停用车辆自动移除；仅允许通行区域且勾选支持申请时追加 " 支持申请主体申请该区域 "  |
...
- 筛选：报告名称、报告周期
- 列表：报告名称、报告周期、关键数据概览、发布机构、发布时间、操作（查看详情/下载）
...
| 围栏类型 | 选择 " 禁止通行区域 " | 隐藏限速配置、支持申请字段；地图样式强制红色虚线（无视状态） |
...
3. **终态不可逆**：已过期、已退回、已撤回、已终止、已变更为终态或半终态，进入后不可再回退到审批中状态。
4. **原申请优先**：派生申请（延期/变更/新增车辆）的状态转换不得影响原申请的正常有效性，除非变更通过或原申请被资格终止。
5. **互斥占用**：同一原申请下，任意派生申请处于占用态（草稿/待初审/待专班审核确认/待上传牌照）时，禁止发起新的派生申请。
...
| E02  | 提交申请    | 测试主体      | 草稿/已退回状态下提交申请    |
| E03  | 初审通过    | 第三方专业管理机构 | 初审节点审批通过         |
| E04  | 初审退回    | 第三方专业管理机构 | 初审节点退回申请         |
| E05  | 专班审核通过  | 市工作专班     | 专班审核节点通过         |
| E06  | 专班审核退回  | 市工作专班     | 专班审核节点退回         |
| E07  | 专家评审通过  | 专家评审组     | 专家评审节点通过（仅初次申请）  |
| E08  | 专家评审退回  | 专家评审组     | 专家评审节点退回         |
| E09  | 专题会审议通过 | 专题会审议     | 专题会审议节点通过（仅初次申请） |
| E10  | 专题会审议退回 | 专题会审议     | 专题会审议节点退回        |
| E11  | 上传牌照    | 测试主体/交管部门 | 上传或录入临时牌照信息      |
...
| 当前状态   | 事件      | 下一状态   | 说明               |
| ------ | ------- | ------ | ---------------- |
| 草稿     | 提交申请    | 待初审    | 校验必填字段与必传材料      |
| 草稿     | 删除      | 已删除    | 仅支持逻辑删除，见 4.2 节  |
| 待初审    | 初审通过    | 待专班审核  | 初次/示范/商业化试点/派生申请 |
| 待初审    | 初审退回    | 已退回    | 需填写审批意见          |
| 待初审    | 撤回申请    | 已撤回    | 下一节点未处理前可操作      |
| 待专班审核  | 专班审核通过  | 待专家评审  | 仅初次申请            |
| 待专班审核  | 专班审核通过  | 待上传牌照  | 仅延期/变更/新增车辆      |
| 待专班审核  | 专班审核退回  | 已退回    | 需填写审批意见          |
| 待专家评审  | 专家评审通过  | 待专题会审议 | 仅初次申请            |
| 待专家评审  | 专家评审退回  | 已退回    | 需填写审批意见          |
| 待专题会审议 | 专题会审议通过 | 待上传牌照  | 仅初次申请            |
| 待专题会审议 | 专题会审议退回 | 已退回    | 需填写审批意见          |
| 待上传牌照  | 上传牌照    | 已生效    | 牌照信息录入后生效        |
| 已生效    | 到期      | 已过期    | 系统定时任务触发         |
| 已生效    | 资格终止    | 已终止    | 市工作专班发起或系统自动触发   |
| 已生效    | 变更覆盖    | 已变更    | 变更申请通过后即时生效      |
| 已退回    | 重新提交    | 待初审    | 修改后重新提交          |
| 已撤回    | 重新提交    | 待初审    | 用户可再次提交          |
...
| 场景 | 原申请状态 | 派生申请状态 | 说明 |
| ---- | ---------- | ------------ | ---- |
| 派生申请审批中 | 保持已生效 | 草稿/待初审/待专班审核确认/待上传牌照 | 不影响企业正常运营 |
| 延期通过 | 到期后转已过期 | 已生效 | 从原到期日 +1 天生效，不设宽限期 |
| 变更通过 | 即时转已变更 | 已生效 | 有效期从变更生效日重新计算 |
| 新增车辆通过 | 保持已生效 | 已生效（不形成独立资格） | 新增车辆并入原申请清单 |
| 派生申请退回 | 保持已生效 | 已退回 | 已退回不占用互斥锁 |
...
| 状态 | 说明 | 可操作按钮 |
| ---- | ---- | ---------- |
| 待处理 | 用户提交后初始状态 | 受理回复、关闭 |
| 处理中 | 已首次回复，问题处理中 | 继续回复、关闭 |
| 已处理 | 问题彻底解决并结单 | 仅关闭/查看 |
| 已关闭 | 反馈被关闭 | 无 |

**转换规则：**

- 首次受理回复后，状态从「待处理」自动更新为「处理中」。
- 「处理中」状态下可多次追加回复。
- 受理人将处理状态切换为「已处理」并提交回复后，反馈关闭。
- 已处理的反馈不可再次回复。
...
3. **终态保护**：已过期、已退回、已撤回、已终止、已变更状态不可再直接通过业务操作回退，仅支持查看、导出、详情展示。
...
- [ ] 16 项申请材料列表展示（材料 1 营业执照自动调取，无需上传），其余必填材料未上传时红色提示
- [ ] 必传材料不完整时阻止提交
...
- [ ] 顶部操作按钮为 " 导出 "，按当前筛选条件导出 Excel（见附录 A.3）
...
- [ ] 视频视图可展示选中车辆的主画面，摄像头选择后主画面和当前摄像头标识同步更新；截图和本地录像均作用于指定摄像头画面；离线车辆显示视频流不可用
- [ ] 历史回放支持日期与时间范围查询、进度定位、播放/暂停、倍速和回到实时，不展示录像片段数量，界面不提供录像导出
- [ ] 同时打开轨迹与视频回放时，拖动任一时间轴均使另一视图定位到相同时间；关闭轨迹回放后视频回放可独立继续
- [ ] 车辆运行监测的车辆列表由统一车辆数据生成，搜索和选择结果能驱动视频工作台
...
- [ ] 新建围栏分两步：填写基本信息（围栏名称、围栏类型、电子围栏状态、限速配置、规则配置）→ 保存后进入图形绘制模式
- [ ] 围栏类型选择允许通行区域时显示限速配置和支持申请勾选，选择禁止通行区域时隐藏以上两项
- [ ] 限速配置：勾选"是否限速"后限速值输入框可用，取消勾选后置灰清空；限速值校验 1~200 km/h
- [ ] 规则配置复选框组：车辆越界报警（默认选中）、已停用车辆自动移除（默认选中）；允许通行区域额外显示"支持申请主体申请该区域"（默认选中）
...
- [ ] 属性编辑弹窗含围栏名称、围栏类型、电子围栏状态、限速配置（仅允许通行区域）、规则配置，所有字段正确回显
- [ ] 切换围栏类型时，限速配置和支持申请字段正确联动显隐
...
- [ ] 右侧信息面板叠加在地图右侧（300px 宽），展示电子围栏状态、围栏类型、围栏长度（待配置矢量显示"—"）、限速配置（仅允许通行区域显示）、规则配置、包含车辆号牌（只读）
