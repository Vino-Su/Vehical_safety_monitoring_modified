...
| V3.4 | 2026-08-05 | 产品经理 | 同日合并更新（F13/F18/F21）：(1) F18 五个 Tab 列表字段统一并新增发布机构列，新建/编辑表单增加发布机构下拉框，创建人改为系统自动获取且只读；(2) F21 信息公开三个 Tab 列表与详情弹窗同步增加发布机构，办事指南"发布部门"统一更名"发布机构"；(3) F21 测试示范报告简化：报告周期枚举统一为月度/季度/年度，列表剔除关键数据概览列，详情弹窗剔除统计图表仅保留正文与附件，移除列表与弹窗的下载入口；(4) F21 公告公示子类型枚举统一为系统公告/重要通知/政策发布/会议通知（与 F18 新建表单子类型一致，列表标签与筛选项共用同一枚举，common.js 下拉选项映射同步修正） | —   |
