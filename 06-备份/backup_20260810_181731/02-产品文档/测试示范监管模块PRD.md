...
| V3.4 | 2026-08-05 | 产品经理 | 同日合并更新（F13/F18/F21）：(1) F18 五个 Tab 列表字段统一并新增发布机构列，新建/编辑表单增加发布机构下拉框，创建人改为系统自动获取且只读；(2) F21 信息公开三个 Tab 列表与详情弹窗同步增加发布机构，办事指南"发布部门"统一更名"发布机构"；(3) F21 测试示范报告简化：报告周期枚举统一为月度/季度/年度，列表剔除关键数据概览列，详情弹窗剔除统计图表仅保留正文与附件，移除列表与弹窗的下载入口；(4) F21 公告公示子类型枚举统一为系统公告/重要通知/政策发布/会议通知（与 F18 新建表单子类型一致，列表标签与筛选项共用同一枚举，common.js 下拉选项映射同步修正）；(5) F19 新建电子围栏弹窗底部按钮更新为三个：取消、保存（仅保存属性信息并关闭弹窗，围栏以"待配置矢量"状态进入列表）、保存并绘制矢量（保存属性信息并自动进入图形绘制页面），替代原单一"保存基本信息并进入图形绘制阶段"按钮；交互流程、弹窗规格与验收标准同步更新；(6) F18 增加待审核信息的审核操作（通过/退回、审核意见）及详情流转时间线，已发布信息的"撤回"统一调整为"下架"，下架后进入已下架列表；(7) F13 轨迹回放查询区新增视频回看入口，右侧视频态同步当前车辆、查询日期、时间范围和时间指针；(8) F13 车辆异常事件回放：车辆信息异常事件卡片提供回放操作，可关闭信息窗并同步开启当前车辆的轨迹与视频回放，回放窗口为事件开始前 1 分钟至事件开始时刻，地图同步展示事件摘要；摘要提供退出回放操作，退出后关闭轨迹回放、视频恢复实时态并切回异常事件视图 | —   |

> V3.4 同日合并项补充：管理端采用统一模块导航；页面上下文由当前模块和功能菜单确定，取消路径式面包屑及独立页面标题；原页面操作入口保持可用，并按当前查询或功能分组呈现。

---

> F13 补充：车辆信息窗标题调整为“车牌号 - 所属企业名称 - 在线状态”，车辆状态页签更新为实时车辆状态字段与按需展示的当前异常事件。

> V3.5（2026-08-08）：F13 监控一张图增加行政区车辆聚合与下钻。全市视角汇总各行政区车辆及异常事件，点击区域聚合结果后进入该行政区的车辆分布；从车辆列表、车辆搜索和异常事件入口选中单车时，地图直接定位并进入单车查看层级。行政区映射与演示车辆点位按实际空间关系维护，避免不同区域数据集中在同一位置；轨迹回放控制组与视频历史回放统一为播放/暂停、倍速和时间轴定位，移除起点/终点及固定一小时跳转按钮。

> V3.6（2026-08-10）：F14 车辆运行监测的视频监控 Tab 移除车辆列表内的快速检索入口和无业务语义的标识，列表直接展示统一车辆数据的车牌号、所属企业、应用类型和运行状态，保留选车驱动视频工作台的联动能力；运行数据 Tab 的列表字段更新为临牌号、VIN码、所属企业、数据采集时间、经度、纬度、车辆驾驶模式、车辆速度、档位，详情补充车辆动态、车身信号与远程控制数据字段；自动驾驶状态 Tab 的列表字段更新为临牌号、VIN码、所属企业、数据采集时间、车辆驾驶模式、接管状态。F22 交通违法行为记录名称调整为”交通违法信息”。F13 轨迹回放与视频历史回放的时间轴精度从分钟升级到秒，当前时间显示、进度条滑块和时间范围筛选均支持秒级定位。
...
4. **数据列表区**：通知公告编号、标题、信息类型、发布机构、创建人、目标受众、发布状态、最后修改时间/创建时间、操作按钮（全部 Tab 页签——全部/草稿箱/待审核/已退回/已发布/已下架——列表字段完全一致；操作按钮按发布状态动态显示：草稿箱显示编辑/删除，待审核显示查看/审核/撤回，已退回显示查看/编辑/删除，已发布显示查看/编辑/下架，已下架仅显示查看）
...
2. 审核通过后信息即时进入“已发布”，审核退回后进入“已退回”；已退回信息可编辑并再次提交审核。待审核记录的创建人可撤回申请，撤回后返回草稿箱。
...
2. 填写围栏名称、围栏类型（允许通行区域 / 禁止通行区域）、电子围栏状态、限速配置（仅允许通行区域时显示）、规则配置后，弹窗底部提供三个按钮，用户二选一完成保存：
...
- [ ] 新建围栏分两步：填写基本信息（围栏名称、围栏类型、电子围栏状态、限速配置、规则配置）→ 弹窗底部三个按钮："保存"仅保存属性信息并关闭弹窗（围栏以"待配置矢量"状态出现在列表），"保存并绘制矢量"保存属性信息并自动进入图形绘制模式
