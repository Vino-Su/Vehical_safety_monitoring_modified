...
var VideoWorkbench=(function(){
  ...
  function buildSegments(v,timeFrom,timeTo){
    var fromSec=timeToSeconds(timeFrom),toSec=timeToSeconds(timeTo);
    var segs=[],start=fromSec;
    while(start<toSec){
      var end=Math.min(start+300,toSec);
      segs.push({
        from:secToTime(start),
        to:secToTime(end),
  ...
  function cameraActionsHtml(st,cam,camIndex){
    return '<div class="vw-camera-actions">'+
      '<button type="button" class="vw-camera-action" title="截图 '+cam.name+'" aria-label="截图 '+cam.name+'" onclick="event.stopPropagation();VideoWorkbench.snapshot(\''+st.id+'\','+camIndex+')">截图</button>'+
      '<button type="button" class="vw-camera-action" title="录像 '+cam.name+'" aria-label="录像 '+cam.name+'" onclick="event.stopPropagation();VideoWorkbench.record(\''+st.id+'\','+camIndex+')">录像</button>'+
    '</div>';
  }
  ...
  function renderProgressBar(st){
    ...
    var pts=st.segments||[];
    var total=timeToSeconds(st.endTime)-timeToSeconds(st.startTime)||1;
    var cur=timeToSeconds(st.currentTime)-timeToSeconds(st.startTime);
    var pct=Math.max(0,Math.min(100,cur/total*100));
    var segMarks=pts.map(function(s){
      var sp=(timeToSeconds(s.from)-timeToSeconds(st.startTime))/total*100;
    ...
    return '<div class="vw-playback">'+
      ...
      '<div class="vw-progress-wrap">'+
        '<input type="range" class="vw-range" min="'+timeToSeconds(st.startTime)+'" max="'+timeToSeconds(st.endTime)+'" value="'+timeToSeconds(st.currentTime)+'" style="width:100%" oninput="VideoWorkbench.seekPlayback(\''+st.id+'\',parseInt(this.value))">'+
      ...
      '<div class="vw-playback-actions">'+
        ...
        '<select class="ant-input vw-speed-select" onchange="VideoWorkbench.setSpeed(\''+st.id+'\',this.value)"><option value="1">1x</option><option value="2">2x</option><option value="4">4x</option><option value="8">8x</option></select>'+
      '</div>'+
    '</div>';
  }
  ...
  function renderPlaybackQuery(st){
    return '<div class="vw-history-query">'+
      ...
      '<div class="vw-history-times"><input class="ant-input" type="time" step="1" value="'+st.startTime+'" aria-label="回放开始时间" onchange="VideoWorkbench.setPlaybackRange(\''+st.id+'\',this.value,null)"><span>至</span><input class="ant-input" type="time" step="1" value="'+st.endTime+'" aria-label="回放结束时间" onchange="VideoWorkbench.setPlaybackRange(\''+st.id+'\',null,this.value)"></div>'+
  ...
  function mount(opts){
    var st={
      ...
      currentTime:opts.currentTime||'08:00:00',
      startTime:opts.startTime||'08:00:00',
      endTime:opts.endTime||'14:32:00',
  ...
  function enterPlayback(id,opts){
    ...
    st.startTime=(opts&&opts.startTime)||st.startTime||'08:00:00';
    st.endTime=(opts&&opts.endTime)||st.endTime||'14:32:00';
  ...
  function setPlaybackRange(id,start,end){
    ...
    if(timeToSeconds(st.endTime)<=timeToSeconds(st.startTime)){showFeedback('结束时间应晚于开始时间','error');return}
    if(timeToSeconds(st.currentTime)<timeToSeconds(st.startTime)||timeToSeconds(st.currentTime)>timeToSeconds(st.endTime))st.currentTime=st.startTime;
  }
  ...
  function seekPlayback(id,seconds){
    var st=instances[id];if(!st)return;
    var tStr=secToTime(seconds);
    st.currentTime=tStr;
  ...
  function togglePlay(id){
    ...
    if(st.playing){
      ...
      st.timer=setInterval(function(){
        var m=timeToSeconds(st.currentTime)+1;
        if(m>=timeToSeconds(st.endTime)){st.playing=false;if(btn)btn.innerHTML='▶ 播放';clearInterval(st.timer);st.timer=null;return}
        seekPlayback(id,m);
      },1000/st.speed);
    }else{
