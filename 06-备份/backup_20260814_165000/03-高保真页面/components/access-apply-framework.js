(function (window, document) {
  ...
  function formatChanges(record) {
    ...
    return keys.map(function (key) {
      ...
      return '<div class="aaf-detail-change"><div class="aaf-detail-change-name">' + esc(change.label || key) + '</div><div class="aaf-detail-pair"><span>原值</span><span class="aaf-detail-old">' + esc(change.oldValue || '-') + '</span></div><div class="aaf-detail-pair"><span>变更后</span><span class="aaf-detail-new">' + esc(change.newValue || '-') + '</span></div>' + (detailTable ? '<div class="aaf-detail-structured"><div class="aaf-detail-structured-head"><span>对象变更明细</span><span>' + esc(itemSummary) + '</span></div>' + detailTable + '</div>' : '') + '</div>';
    }).join('');
  }
  ...
  function addPrecondition(source) {
    if (!state.config || typeof state.config.getAddVehiclePrecondition !== 'function') return '';
    var text = state.config.getAddVehiclePrecondition(source, state.record);
    return text ? '<section class="aaf-section"><h4 class="aaf-section-title">新增车辆前置条件</h4><div class="aaf-precondition">' + esc(text) + '</div></section>' : '';
  }
  ...
  function addDetailHtml(record) {
    ...
    var title = (state.config && state.config.addVehicleTitle) || '新增相同配置车辆';
    return derivedDetailHtml(title, record, source, [detailSection('申请信息', '<section class="aaf-subsection"><h5 class="aaf-subsection-title">新增车辆</h5>' + addDetailVehicleTable(record, source) + '</section><div class="aaf-readonly"><div class="aaf-readonly-label">新增原因</div><div class="aaf-readonly-value">' + esc(record.addReason || record.reason || '-') + '</div></div>'), detailSection('申请材料', '<div class="detail-attachment-panel operate-detail-group aaf-detail-attachments"><section class="attachment-group"><div class="attachment-group-title">本次申请文件</div>' + addDetailAttachmentRows(record) + '</section></div>')]);
  }
  ...
  var api = {
    ...
    openAddVehicle: function (config, recordOrId) {
      ...
      var initialFiles = Object.assign({}, (state.record.attachments || {}));
      state.add = { source: source, vehicles: (state.record.addVehicles || []).slice(), reason: state.record.addReason || state.record.reason || '', files: {} };
      addAttachmentItems().forEach(function (item) { state.add.files[item.key] = initialFiles[item.key] || ''; });
