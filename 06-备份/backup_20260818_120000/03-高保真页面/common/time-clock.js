...
// "HH:mm:ss" 转秒数，供按时间定位使用
function timeToSeconds(t){var p=String(t||'').split(':');return(parseInt(p[0])||0)*3600+(parseInt(p[1])||0)*60+(parseInt(p[2])||0)}

// 秒数转 "HH:mm:ss"，供时间显示使用
function secToTime(sec){var h=Math.floor(sec/3600);var m=Math.floor((sec%3600)/60);var s=sec%60;return String(h).padStart(2,'0')+':'+String(m).padStart(2,'0')+':'+String(s).padStart(2,'0')}
