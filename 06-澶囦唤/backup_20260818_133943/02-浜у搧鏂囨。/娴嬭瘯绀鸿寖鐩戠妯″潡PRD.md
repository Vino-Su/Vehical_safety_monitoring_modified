...
#### 3.1.7 信息快照
企业详情提供“信息快照”查看入口。列表按快照时间倒序展示快照编号、快照时间、修改人和查看操作；用户点击查看后，以只读形式展示该企业在该快照时的完整登记信息及营业执照附件。新增登记、保存编辑或更新附件时自动生成快照；历史快照不可编辑、删除或恢复。准入申请引用企业时记录对应快照编号，后续企业登记信息变更不得影响已提交申请的历史依据。
...
#### 3.2.7 信息快照
车辆详情提供“信息快照”查看入口。列表按快照时间倒序展示快照编号、快照时间、修改人和查看操作；用户点击查看后，以只读形式展示该车辆在该快照时的完整登记信息及关联附件。新增登记、保存编辑、状态调整或更新附件时自动生成快照；历史快照不可编辑、删除或恢复。准入申请引用车辆时记录对应快照编号，后续车辆登记信息变更不得影响已提交申请的历史依据。
...
#### 3.3.7 信息快照
人员详情提供“信息快照”查看入口。列表按快照时间倒序展示快照编号、快照时间、修改人和查看操作；用户点击查看后，以只读形式展示该人员在该快照时的完整登记信息及关联附件。新增登记、保存编辑或更新附件时自动生成快照；历史快照不可编辑、删除或恢复。准入申请引用人员时记录对应快照编号，后续人员登记信息变更不得影响已提交申请的历史依据。
...
| 初次申请、涉及新增或替换车辆的变更申请、新增相同配置车辆申请 | 待上传牌照  | 审批通过，用户尚未登记本次车辆的临时牌照信息              | 橙色   | ant-tag-warning    |
| 初次申请、延期申请、变更申请、新增相同配置申请 | 已生效    | 需登记牌照的车辆均已完成登记，或审批通过后无需登记牌照，且当前处于申请有效期内 | 绿色   | ant-tag-success    |
...
```
延期/无车辆变更：草稿 → 待初审 → 待专班审核 → 已生效 → 已过期
车辆变更/新增车辆：草稿 → 待初审 → 待专班审核 → 待上传牌照 → 已生效 → 已过期
```
...
- 新增车辆通过：原申请状态不变，本次新增车辆完成牌照登记后并入原申请清单
- 派生申请退回/撤回：原申请不受影响
...
- 新增车辆通过后进入 " 待上传牌照 "，原申请状态不变；仅本次新增车辆完成牌照登记后并入原申请车辆清单并标注来源。
- 派生申请退回/撤回：原申请不受影响；
- 原申请被资格终止：其下所有进行中的派生申请联动置 " 已终止 "；
...
- 申请列表和申请详情车辆信息提供同一登记入口；登记时只录入临时牌照号与有效期，不上传照片或其他凭证，不新增待办、通知、独立页面或二次审核。
- 用户须一次完成全部待登记车辆的牌照号和有效期；系统校验必填、牌照号格式及同一申请内重复，校验通过后申请自动生效。已生效申请仅只读展示牌照信息。
...
| S06  | 待上传牌照  | 审批通过，等待用户登记适用范围内车辆的临时牌照               | 否    |
| S07  | 已生效    | 所有待登记车辆已完成临时牌照登记，或审批通过后无需登记牌照，申请进入有效期             | 否    |
...
stateDiagram-v2
    ...
    待初审 --> 已退回 : 初审退回
    待初审 --> 已撤回 : 撤回申请
    待专班审核 --> 已生效 : 通过（延期/不涉及车辆的变更）
    待专班审核 --> 待上传牌照 : 通过（新增或替换车辆）
    待专班审核 --> 已退回 : 专班审核退回
    待上传牌照 --> 已生效 : 上传牌照
    已生效 --> 已过期 : 到期
    已生效 --> 已终止 : 资格终止
...
| 待专班审核  | 专班审核通过  | 已生效  | 延期、无车辆变更      |
| 待专班审核  | 专班审核通过  | 待上传牌照  | 仅涉及新增或替换车辆的变更、以及新增相同配置车辆      |
...
| 待上传牌照  | 登记临时牌照    | 已生效    | 全部待登记车辆的牌照号和有效期录入并校验通过后生效        |
| 已生效    | 到期      | 已过期    | 系统定时任务触发         |
...
| 场景 | 原申请状态 | 派生申请状态 | 说明 |
| ---- | ---------- | ------------ | ---- |
| 派生申请审批中 | 保持已生效 | 草稿/待初审/待专班审核确认/待上传牌照 | 仅车辆变更和新增车辆可进入待上传牌照，不影响企业正常运营 |
| 延期通过 | 到期后转已过期 | 已生效 | 从原到期日 +1 天生效，不设宽限期 |
| 不涉及车辆的变更通过 | 即时转已变更 | 已生效 | 有效期从变更生效日重新计算 |
| 涉及新增或替换车辆的变更通过 | 保持已生效 | 待上传牌照 | 登记完成后原申请转已变更，变更申请生效 |
| 新增车辆通过 | 保持已生效 | 待上传牌照/已生效（不形成独立资格） | 仅新增车辆登记完成后并入原申请清单 |
...
| 状态名称        | 说明                      | 是否终态 |
| ----------- | ----------------------- | ---- |
| 事故信息待审核     | 事故信息已提交，等待第三方服务机构审核     | 否    |
| 事故报告待提交     | 事故信息已审核通过，等待用户上传事故报告    | 否    |
| 事故报告待审核     | 用户已上传事故报告，等待第三方服务机构审核   | 否    |
| 事故分析报告待提交   | 事故报告审核已通过，等待用户上传事故分析报告  | 否    |
| 事故分析报告待初审   | 用户已上传事故分析报告，等待第三方服务机构初审 | 否    |
| 事故分析报告待专班审核 | 初审通过，等待工作专班进行审核         | 否    |
| 已完成         | 工作专班审核通过，事故上报完成         | 是    |
| 已退回         | 审批节点被退回，需修改后重新提交        | 是    |
| 已撤回         | 用户在审批前主动撤回申请            | 是    |
### 5.9 状态机通用约束
...
- [ ] 初次申请、涉及新增或替换车辆的变更申请、以及新增相同配置车辆申请审批通过后，可从列表或详情车辆信息进入同一临时牌照登记功能；仅填写牌照号与有效期，无需上传凭证，全部待登记车辆校验通过后自动生效；延期及不涉及车辆的变更不显示登记入口，已生效详情只读展示牌照信息
