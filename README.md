...
### V3.4 · 2026-08-05 — 地图视频工作台优化；信息发布与信息公开字段调整

**变更范围**：监控一张图的视频态增加车辆选择，压缩右侧工作台的操作层级，并明确历史录像不提供导出；信息发布管理五个 Tab 列表字段统一并新增发布机构字段，信息公开列表同步展示发布机构；信息公开-测试示范报告展示简化（报告周期枚举调整、剔除关键数据概览与统计图表）。

**主要内容**：
- **车辆切换**：视频工作台可直接按车牌选择统一车辆数据，视频源、地图选中车辆与地图视角同步更新。
- **回放范围**：保留日期时段查询、片段定位、播放控制与回到实时，移除录像导出入口及权限反馈。
- **操作边界**：截图和本地录像直接在对应摄像头画面内执行；全屏内置退出及单路、4路、9路布局切换，回放不显示录像片段数量，并与回到实时使用同一切换入口。
- **信息发布管理**：全部/草稿箱/待审核/已发布/已下架五个 Tab 列表字段与"全部"Tab 完全一致（取消各状态 Tab 差异化时间列）；新建/编辑表单新增"发布机构"下拉框（市工作专班办公室/市交通运输局/市交管部门/安全管理办公室/平台技术支持组），"创建人"改为系统自动获取当前登录用户、只读不可编辑；列表与详情弹窗同步展示发布机构（列表共 9 列）。
- **信息公开**：公告公示、测试示范报告列表新增"发布机构"列，办事指南"发布部门"统一更名为"发布机构"，三类详情弹窗同步展示。
- **测试示范报告简化**：报告周期枚举统一为月度/季度/年度（信息发布管理同步移除半年度）；列表剔除"关键数据概览"列；详情弹窗剔除关键数据概览卡片及里程统计/载客统计/车辆类型分布/企业排名等图表，仅保留基本信息、报告正文与附件（信息发布管理新建表单同步移除"关键数据概览"字段）；列表操作列与详情弹窗底部的"下载"入口均移除，仅保留查看详情。

### V3.3 · 2026-08-04 — 视频、轨迹与数据统一时间轴

**变更范围**：监控一张图与车辆运行监测共用视频工作台、车辆数据和时间指针，补齐历史录像与地图轨迹双向联动。

**主要内容**：
- **统一数据与时间**：两页复用 15 辆车辆和 6 路摄像头配置；轨迹时间改变会定位视频画面，视频回放定位会反向更新地图轨迹与运行数据时刻。
- **视频工作台**：实时与历史录像共用工作台，支持主画面摄像头切换、多路查看、日期时间范围查询、片段定位、倍速和回到实时。
- **监控一张图**：右侧异常事件态与视频态可切换；视频态采用主画面与摄像头网格的组合，轨迹回放期间自动联动视频并收起车辆抽屉。
- **车辆运行监测**：修复视频 Tab 脚本初始化；车辆列表由统一数据源生成，支持全部车辆搜索、选择与视频驱动。

**修改文件**：
- `03-高保真页面/common/video-workbench.js`
- `03-高保真页面/common/video-workbench.css`
- `03-高保真页面/monitor/vehicle-monitor/monitor-map.html`
- `03-高保真页面/monitor/vehicle-monitor/running.html`
- `02-产品文档/测试示范监管模块PRD.md`
- `README.md`

### V2.9 · 2026-07-06 — 资格终止管理页面与数据字典对齐 PRD 5.10

**变更范围**：资格终止管理页面按 `04-交付文档/测试示范监管模块PRD-V1.md` 5.10 章节全面对齐；同步修正共享下拉与数据字典。

**主要内容**：
- **列表区（5.10.1）**：查询条件对齐 PRD 5 项（申请主体、终止类型、触发方式、终止状态、终止日期范围）；表头新增「操作人」列（自动触发标识为"系统"，手动显示具体操作方）；默认按终止日期倒序。
- **发起资格终止弹窗（5.10.3）**：改为「申请主体下拉 → 自动带出关联有效申请流程」联动模型，支持多选（默认全选）与全选/反选；新增「附件上传」（非必填，复用项目通用按钮样式）；按钮文案改为「确认终止」；补 4 项必填校验与成功/失败 Toast，校验失败弹窗不关闭。
- **资格终止详情（5.10.4）**：新增「附件」「关联申请流程」列表区块（主申请置顶 + 同主体其他申请合并）；「操作方」更名为「操作人」；保留车辆/道路/材料/审批流/日志等丰富展示。
- **数据字典（举一反三）**：`common.js` 共享下拉「终止类型」由旧值（主动/强制/到期终止）修正为 PRD 6 种（车辆与声明不符、重大安全风险、严重违法、交通事故、号牌到期/撤销、道路关闭）；新增「终止状态」选项；`platform/dict/index.html` 中 `TERMINATE_TYPE` 字典 count 3→6、版本 V1→V2。

**修改文件**：
- `03-高保真页面/monitor/access-approve/terminate.html`
- `03-高保真页面/common.js`
- `03-高保真页面/platform/dict/index.html`
- `README.md`

**自测截图**：`07-bugs/terminate_full_2026-07-06_*.png`（1366×1200 大视口，逐行核验操作人列"市工作专班/系统"分布与终止日期倒序）

### V2.10 · 2026-07-09 — 材料目录配置 / 目录版本管理 Tab 前端展示优化

**变更范围**：「材料目录配置」页面的「目录版本管理」子 Tab 前端展示优化（不动业务逻辑、不动 PRD）。

**主要内容**：
- **移除版本状态下拉筛选**：删除右侧 `.actions` 块中的「版本状态」`<select>` 筛选与配套「重置」按钮，标题区仅保留类型标题 + 「已发布+历史版本数」统计文案；同步清理 `versionFilter` 全局变量、`changeVersionFilter()` / `resetVersionFilter()` 死代码及 `selectVersionType()` 中的相关引用。
- **表格列宽重排**（colgroup）：目录版本号 120→110、版本状态 120→120、材料数量 90→100、发布时间 140→170、操作人 100→120、操作 自适应→`min-width:220px`。
- **表头与数据列对齐规范**：按 `UI_design.md` 规则补齐对齐 class — 目录版本号 / 发布时间 `col-code`（居中）、版本状态 `col-status`（居中）、材料数量 `col-num`（右对齐）、操作 `col-action`（居中）；「材料数量」列名补充单位「（项）」。

**修改文件**：
- `03-高保真页面/monitor/approve-config/config.html`
- `README.md`

**自测截图**：`07-bugs/material_version_2026-07-09_*.png`（1366×768、1920×1080 双分辨率）

### V2.8 · 2026-07-01 — 在线反馈拆分为公众侧与管理侧

**变更范围**：测试示范监管模块新增"在线反馈管理"工作台；现有"在线反馈"页调整为公众侧；PRD V2.8。

**主要内容**：
- **新增** `03-高保真页面/monitor/service/feedback-manage.html`：**在线反馈管理**工作台（admin 可见），含 5 个 Tab（全部/待处理/处理中/已处理/已关闭）、列表、详情、受理回复/继续回复/关闭弹窗，列表新增"回复次数/最新回复时间"字段（由回复列表前端计算）。
- **调整** `03-高保真页面/monitor/public/feedback.html`：移除"受理机构"列、移除"受理回复/继续回复/关闭"按钮，admin 进入该页面仅可查看，无管理操作。
- **菜单** `03-高保真页面/common.js`：在"测试服务管理"分组下新增菜单项 `在线反馈管理 → feedback-manage.html`，roles=`['admin']`。
- **PRD V2.8**：F11 在线反馈管理 拆分为 **3.11.1 在线反馈（公众侧）** 与 **3.11.7 在线反馈管理（管理侧）**；新增 5 个 Tab 设计、列表字段、交互流程、弹窗规格、权限矩阵；验收标准新增 8.8 节；状态机（4.15.4）、编号规则（4.5）、数据结构保持不变；本期不做统计卡片/批量操作/转派/认领/内部备注/消息推送。

**修改文件**：
- `03-高保真页面/monitor/service/feedback-manage.html`（新增）
- `03-高保真页面/monitor/public/feedback.html`
- `03-高保真页面/common.js`
- `02-产品文档/测试示范监管模块PRD.md`
- `README.md`

**自测截图**：`07-bugs/feedback_manage_*_20260701.png`

### V2.7 · 2026-06-30 — 三类准入申请材料清单首位新增"主体单位营业执照"

**变更范围**：道路测试/示范应用/商业化试点三类准入申请的新建弹窗、详情页与 PRD。

**主要内容**：
- 申请材料清单首位新增**材料1：主体单位营业执照**（道路测试/示范应用/商业化试点三套分别命名为"道路测试/示范应用/商业化试点主体单位营业执照"），由系统自动从「企业信息登记」模块调取展示，无需手动上传。
- 原材料依次后推：道路测试 **15 → 16**、示范应用 **14 → 15**、商业化试点 **16 → 17**。
- 新建申请弹窗中：材料1 渲染为"已上传"形态（绿色✓ + 营业执照.pdf + 预览/下载），不显示上传按钮、不显示"未上传文件"红字，与已上传材料展示完全一致。
- 详情页（含派生申请折叠区、延期/变更原申请材料区）：因 `materialList` 数据更新，材料1 自动以"已上传"形态展示，无需改动渲染逻辑。
- PRD（`02-产品文档/测试示范监管模块PRD.md` V2.7）：三处清单表与注解、验收标准、版本记录同步更新。

**修改文件**：
- `03-高保真页面/monitor/access-apply/road-test.html`
- `03-高保真页面/monitor/access-apply/demo-apply.html`
- `03-高保真页面/monitor/access-apply/demo-operate.html`
- `02-产品文档/测试示范监管模块PRD.md`

**自测截图**：`07-bugs/road_test_*_full_*.png`、`demo_apply_*_full_*.png`、`demo_operate_*_full_*.png`（弹窗使用 1440×3000 大视口截取，确保"申请材料"区与材料1 完整可见）
...
| 测试示范监管模块PRD | `02-产品文档/测试示范监管模块PRD.md` | V3.4 | 待确认 |
