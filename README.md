...
| 模块 | 菜单 Key | 目录 | 页面数 | 需求文档 |
|------|----------|------|--------|----------|
| 开放道路资源管理 | `road` | `UI/road/` | 6 | `需求规格说明书/road_requirements.md` |
| 测试示范监管 | `monitor` | `UI/monitor/` | 20 | `需求规格说明书/test_requirement.md` |
| 监测数据分析 | `analysis` | `UI/data-analysis/` | 21 | `需求规格说明书/data_analysis_req.md` |
| 平台管理 | `platform` | `UI/platform/` | 2 | `需求规格说明书/platform_req.md` |
...
```
智能网联汽车安全监测平台/
├── index.html                          ← 入口文件，跳转至 UI/layout.html
├── README.md                           ← 本文件
├── 公司名称模糊化映射表.md               ← 公司名称脱敏映射规则（待执行）
├── .gitignore
├── .gitattributes
│
...
│   └── skills/                         ← 自定义技能
│
├── 需求规格说明书/                       ← 需求文档（开发的核心依据）
│   ├── road_requirements.md            ← 开放道路资源管理模块需求（30 KB）
│   ├── test_requirement.md             ← 测试示范监管模块需求（68 KB）
│   ├── data_analysis_req.md            ← 监测数据分析模块需求（28 KB）
│   └── platform_req.md                 ← 平台管理模块需求（11 KB）
│
├── reference/                          ← 参考原型与文档
│   ├── *.html                          ← 参考页面原型（23 个文件）
│   ├── 交通事故报告模板.docx
...
| 审批流程 | 时间线组件，串联审批节点：第三方初审 → 联席小组审批 → 临牌回传 |
...
4. **路径计算**：`common.js` 通过 `getBasePath()` 自动计算相对路径深度，页面在 UI 子目录下任意层级均可正确导航
5. **弹窗嵌套**：`openModal()` 支持多层嵌套，内层弹窗打开时外层自动隐藏，关闭内层后外层自动恢复
6. **分页默认**：所有列表页默认每页 10 条，支持分页
7. **备份机制**：每次执行任务前，会将当前 UI 目录完整备份到 `backup/` 目录（仅保留最近一次）
8. **需求对齐**：所有页面需与 `需求规格说明书/` 中的文档保持一致，不得遗漏功能点
...
| 文档 | 路径 | 内容概要 |
|------|------|----------|
| 开放道路资源管理 | `需求规格说明书/road_requirements.md` | 道路建档字段清单、动态管控状态变更校验、地图可视化图层、评估模型配置、区域统计 |
| 测试示范监管 | `需求规格说明书/test_requirement.md` | 准入信息登记字段、三类准入申请流程与材料清单、串联审批流程、监控一张图、电子围栏、测试评价 |
| 监测数据分析 | `需求规格说明书/data_analysis_req.md` | 测试数据总览、告警统计5Tab架构、数据查询检索、事故解析/成因/特征/研判、测试示范专题分析 |
| 平台管理 | `需求规格说明书/platform_req.md` | 数据字典CRUD与版本管理、操作日志查询与审计、日志详情弹窗字段 |
